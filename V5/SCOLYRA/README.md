...
> ⚠️ **État du projet : V5 — Stripe réellement implémenté (Checkout,
> webhook, portail de facturation), Documents/Projets/Révisions
> persistés en base, vérification du représentant légal fonctionnelle
> de bout en bout. Deux bugs bloquants trouvés lors d'un premier vrai
> test ont été corrigés (schéma Prisma, image Docker MinIO).** Voir
> [`docs/STATUS.md`](docs/STATUS.md) pour le détail exact de ce qui est
> réellement implémenté et de ce qui reste à faire. **Le reste n'a
> jamais été exécuté** dans l'environnement où le code a été généré
> (pas de Node/réseau) — Stripe et les routes de cette livraison en
> particulier restent à tester en conditions réelles.
