...
> ⚠️ **État du projet : V4 — sécurité durcie (rate limiting, en-têtes
> HTTP), RGPD réellement implémenté (export/suppression de compte),
> panneau admin pour piloter les fonctionnalités Premium.** Voir
> [`docs/STATUS.md`](docs/STATUS.md) pour le détail exact de ce qui est
> réellement implémenté et de ce qui reste à faire. **Rien de tout ceci
> n'a été exécuté** dans l'environnement où le code a été généré (pas
> de Node/réseau) — teste `pnpm db:migrate` et `pnpm dev` en premier.
