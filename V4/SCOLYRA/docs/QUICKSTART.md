...
## Configurer Stripe (optionnel, mode TEST uniquement)

⚠️ Rappel important (voir [PAYMENTS.md](PAYMENTS.md)) : **le code des
routes Stripe (Checkout, webhooks) n'est pas encore implémenté dans
cette version.** Ce qui suit prépare le terrain (clés, CLI) pour
quand ces routes seront ajoutées — inutile de le faire si tu veux
juste tester le reste de l'app.

1. Crée un compte sur [stripe.com](https://stripe.com) (gratuit).
2. Dans le Dashboard Stripe, reste en **mode Test** (bouton en haut à
   droite — ne jamais utiliser les clés "Live" en développement).
3. Récupère tes clés de test : *Developers → API keys* :
   - `Publishable key` (commence par `pk_test_...`) →
     `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` dans `.env`
   - `Secret key` (commence par `sk_test_...`) → `STRIPE_SECRET_KEY`
4. Installe le [Stripe CLI](https://stripe.com/docs/stripe-cli) pour
   recevoir les webhooks en local (une fois les routes implémentées) :
   ```bash
   stripe login
...
Sans clé, les emails ne sont simplement pas envoyés (à implémenter,
voir STATUS.md). Pour activer :
