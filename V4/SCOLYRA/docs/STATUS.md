...
État :
V4 — durcissement sécurité (rate limiting, en-têtes HTTP, cookies
explicites), RGPD réellement implémenté (export/suppression de
compte, consentement à l'inscription, pages légales), panneau admin
pour piloter dynamiquement quelles fonctionnalités sont Premium. Reste
non branché : Stripe réel, pipeline RAG réel, tests automatisés, pages
Projets/Révisions/Documents encore en état local (non persistées).
...
- Point d'attention prioritaire au premier lancement : `pnpm db:migrate`
  ...
  modèle FeatureFlag de la V4), puis `pnpm seed:demo`, puis tester la
