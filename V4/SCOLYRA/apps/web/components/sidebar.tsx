  ...
  return (
    <aside className="hidden w-72 flex-col bg-ink-900 p-5 md:flex">
      ...
      <div className="mb-4 rounded-xl bg-white/5 p-4">
        <ProgressRing current={overallAverage} target={overallTarget} label={`${firstName} · moyenne générale`} />
      </div>
      ...
      <div className="mb-6 flex items-center gap-2 rounded-xl bg-gradient-to-r from-gold-500/15 to-transparent px-3 py-2">
        ...
        <span className="text-sm text-gold-200">
          <strong className="font-display">{activeDaysThisWeek}</strong>/7 jours actifs cette semaine
        </span>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between border-t border-white/10 pt-4 px-1">
        ...
        <button onClick={() => signOut({ callbackUrl: "/" })} className="focus-ring text-xs text-white/45 hover:text-white/70">
          Déconnexion
        </button>
      </div>
    </aside>
