...
> ⚠️ **État du projet : V7 — quatrième bug bloquant corrigé
> (`extensions = [vector]` utilisé sans la preview feature Prisma
> `postgresqlExtensions`, ce qui faisait aussi échouer les seeds en
> cascade). Toujours pas de validation de bout en bout du pipeline
> complet (install → docker → migrate → seed → dev).** Voir
> [`docs/STATUS.md`](docs/STATUS.md) pour le détail exact. Merci de
> continuer à signaler ce qui casse, étape par étape.
