...
État :
V7 — quatrième bug bloquant trouvé lors d'un vrai test corrigé :
`extensions = [vector]` dans le schéma Prisma sans la preview feature
`postgresqlExtensions` (P1012), ce qui empêchait aussi la génération du
client Prisma et donc faisait échouer les seeds en cascade. V6 :
packages/ui n'avait pas de package.json. V5 : Documents/Projets/
Révisions désormais persistés en base, vérification du représentant
légal implémentée de bout en bout, Stripe réellement implémenté. Reste
non branché : pipeline RAG réel, tests automatisés.
...
- MISE À JOUR IMPORTANTE : ce projet bénéficie enfin de vrais tests
  (merci !), qui ont trouvé et permis de corriger quatre bugs
  bloquants au total : commentaire `/** */` invalide dans le schéma
  Prisma, image Docker MinIO disparue de Docker Hub,
  packages/ui/package.json manquant, et `extensions = [vector]` sans
  la preview feature `postgresqlExtensions` (ce dernier empêchait à
  lui seul la migration ET les deux seeds — voir le détail plus haut
  et dans TROUBLESHOOTING.md). Le pipeline complet (install → docker →
  migrate → seed) n'a par contre toujours pas pu être validé de bout
  en bout puisque chaque test s'est arrêté au premier échec rencontré —
  il peut encore rester un cinquième problème après celui-ci.
